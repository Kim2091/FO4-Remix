...
| `src/semantic_capture.{cpp,h}` | MinHook installation of two `GetRenderPasses` detours (Lighting RVA `0x02172540`, Water RVA `0x021D15A0`) plus `SetupGeometry` (`0x02233730`) / CB-write (`0x022347D0`) hooks that capture the final per-draw world transform uploaded by the engine. Computes PassKeys from `(geometry, property, material)`, adding a quantized transform suffix when the final transform differs from the node transform, stores `DrawableState` in `g_drawableMap`, captures live transforms and `NiAVObject::flags` per fire. `Tick(device)` runs the per-frame resolve loop (freshness-gated, VRAM-gated), the rate-limited TTL sweep, the pending-by-gate breakdown log, and the active-set snapshot. `BuildRemixTransform` converts NiTransform to Remix row-major 3x4 with the X/Y swap. `ClearDrawableMap` is called from `kMessage_PreLoadGame`. |
...
4. **`Resolvers::Lighting::TryResolveStatic`** (`resolvers/lighting_static.cpp:67`):
   ...
   - `BsExtraction::ParseShapeGeometry` parses vertex / index buffers from
     ...
     `BSGeometry::kFlag_FullPrecision` for half-float positions. Dynamic
     morphed positions are read from `BSDynamicTriShape::dynamicVertices`
     only when `GetAsBSDynamicTriShape()` succeeds; plain `BSTriShape`
     descriptors can have a non-zero `szVertex` nibble and must stay on the
     normal vertex buffer path.
