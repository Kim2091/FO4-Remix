...
// ---------------------------------------------------------------------------
// FaceGen morph refresh (2026-07-09): live dynamicVertices snapshot + decode.
//
// LIVE-PROVEN mechanism (frida session, 2026-07-09): during dialogue and
// ambient expressions the engine memcpy's freshly CPU-deformed model-space
// positions into a facegen BSDynamicTriShape's dynamicVertices buffer every
// frame (writer resolved to vcruntime memcpy via MemoryAccessMonitor;
// ~1378/1689 head vertices changed per sampled frame while the NPC talked,
// mouth/eye parts likewise; idle faces stay byte-identical). This supersedes
// the earlier static-analysis claim that the buffer is written exactly once
// at conversion -- that xref enumeration missed the (inlined-lock) writer.
// ---------------------------------------------------------------------------
namespace {

// POD-only SEH stage: reads the live byte size (+0x170), vertex count
// (+0x164, u16) and buffer pointer (+0x180), bounds-checks, and bulk-copies
// into dst when provided. Raw offsets are the decompiler-proven
// BSDynamicTriShape layout (scripts/dynamic_trishape_desc.md); same idiom as
// skinned_meshes.cpp's guarded raw reads.
...
bool BsExtraction::DecodeDynamicPositions(const std::vector<uint8_t>& raw,
    ...
    if (elem <= 12) {
        // half4 position (x, y, z, bitangentX) + tail -- the byte-exact
        // proven facegen layout (same decode as ParseShapeGeometry's
        // dynamic path).
        ...
        // Defensive float3 path for 16-byte elements (none observed).
        for (uint32_t i = 0; i < numVertices; ++i) {
            const float* fp =
